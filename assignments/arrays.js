...
const invSearch = (arr, invNum) => {
   
    for(i=0; i<arr.length; i++){
        if(arr[i].id === invNum){
            console.log(`Car ${arr[i].id} is a ${arr[i].car_year} ${arr[i].car_make} ${arr[i].car_model}`);
        }
    }
};

invSearch(inventory, 33);


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory.length -1;
console.log(`The last car has id ${inventory[lastCar].id} and is a ${inventory[lastCar].car_year} ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`);

// ==== Challenge 3 ====
// The marketing team wants the car models listed alphabetically on the website. Sort all the car model names into alphabetical order and log the results in the console

// const sortAlpha = function(arr){
//     for(let i=0; i<arr.length; i++){
//         carModels.push(arr[i].car_make);
//     };
//     retrun carModels; 
// }
...
//         let modelList = [];
//         for(let i = 0; i<arr; i++){
//             modelList.push(arr[i].car_model);
//         }
//         return modelList;
//     };
//  
// carModels = modelSort(inventory);
...
const sortModels = (arr) => {
    for(let i = arr.length -1; i >= 0; i--){
        for(let j = 1; j<=i; j++){
            if(arr[j-1].car_model>arr[j].car_model){
                let temp = arr[j-1];
                arr[j-1] = arr[j];
                arr[j] = temp;
            }
...
const sortByAge = (arr, modelYear) => {
    let arr2 = [];
    for(let i = 0; i < arr.length; i++){
        if(modelYear > arr[i].car_year){
            arr2.push(arr[i]);
        }
    }
    return arr2;
}
oldCars = sortByAge(inventory, 2000);
// console.log(oldCars); 
...
const makeSearch = (arr, make1, make2) => {
    let arr2 = [];
    for(let i = 0; i < arr.length; i++){
        if(arr[i].car_make.toLowerCase() === make1.toLowerCase()){
            arr2.push(arr[i]);
        } else if(arr[i].car_make.toLowerCase() === make2.toLowerCase()) {
            arr2.push(arr[i]);
        }
    }
    return arr2;
...
BMWAndAudi = makeSearch(inventory, 'BMW', "AUDI");
